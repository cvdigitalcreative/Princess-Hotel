...
class Tulisan extends CI_Controller{
	...
	function index(){
		...
		$y['title'] = 'Princess Hotel || Tulisan';
	...
	function add_tulisan(){
		...
		$y['title'] = 'Princess Hotel || Add Tulisan';
		$this->load->view('admin/v_header',$y);
		$this->load->view('admin/v_sidebar',["side" => 2]);
	...
	function get_edit(){
		...
		$x['kat']=$this->m_kategori->get_all_kategori();
		$y['title'] = 'Princess Hotel || Edit Tulisan';
		$this->load->view('admin/v_header',$y);
		$this->load->view('admin/v_sidebar',["side" => 2]);
	...
	function simpan_tulisan(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							$this->m_tulisan->simpan_tulisan($judul,$isi,$harga,$kategori_id,$kategori_nama,$album_id,$album_nama,$user_id,$user_nama,$gambar,$slug);
	...
	function update_tulisan(){
	            ...
	            if(!empty($_FILES['filefoto']['name']))
	            {
	                if ($this->upload->do_upload('filefoto'))
	                {
							...
							$user_id=$p['pengguna_id'];
							$user_nama=$p['pengguna_nama'];
							$this->m_tulisan->update_tulisan($tulisan_id,$judul,$isi,$harga,$kategori_id,$kategori_nama,$album_id,$album_nama,$user_id,$user_nama,$gambar,$slug);
	                    ...
	                    redirect('Admin/Tulisan');
							...
							$user_id=$p['pengguna_id'];
							$user_nama=$p['pengguna_nama'];
							$this->m_tulisan->update_tulisan_tanpa_img($tulisan_id,$judul,$isi,$harga,$kategori_id,$kategori_nama,$album_id,$album_nama,$user_id,$user_nama,$slug);
							echo $this->session->set_flashdata('msg','info');
							redirect('Admin/Tulisan');
	            } 
